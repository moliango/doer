...
- [ ] 默认关：回复编辑器仍是现在的 Aa/MD 框。
- [ ] 设置里能打开「实验性所见即所得」；再开回复，正文是块列表而不是整篇一个源码框。
- [ ] 段落里打字看不见 `**` 仍能通过工具栏加粗；发出去是合法 `[b]`/`**` Markdown。
- [ ] 标题/引用/列表/代码块有块级外观；导出能 round-trip 常见写法。
- [ ] 含 `[poll]` 的草稿打开实验模式不丢投票 BBCode。
- [ ] 关掉开关后再开回复，回到旧编辑器，草稿还在。

## Out Of Scope

- Discourse 官方 cook JS、往返 cook 门禁。
- 投票/图岛/onebox 作为可点选孤岛。
- 发帖/私信第一期可以仍走旧框（回复先通）。
- 光标处显形（IR）模式。
